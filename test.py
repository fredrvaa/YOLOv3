...
def test(cfg,
    ...
    for batch_i, (imgs, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        for si, pred in enumerate(output):
            ...
            if save_json:
                ...
                root,ds,_,sub,sub2,file_name = str(paths[si]).split('/')
                ds = 'datasets'
                typ = 'annotations'
                coco_data_dir = os.path.join(root,ds,sub,typ)

                with open(f"{coco_data_dir}/{sub2}.json") as cd_file:
                    cd = json.load(cd_file)

                image_id = [x for x in cd['images'] if x['file_name'] == file_name][0]['id']
    ...
    if save_json and map and len(jdict):
        ...
        cocoGt = COCO(glob.glob('../datasets/T/annotations/test.json')[0])  # initialize COCO ground truth api
        ...
        stats = ''
        for s in cocoEval.stats.tolist():
            stats += (str(s)+',')
            
        stats.rstrip(',')
        print(stats)

        with open('coco_stats.txt','w') as coco_stats_file:
            coco_stats_file.write(stats)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='cfg/yolov3-rust.cfg', help='*.cfg path')
    parser.add_argument('--data', type=str, default='data/rust.data', help='*.data path')
    ...
    opt.save_json = opt.save_json or any([x in opt.data for x in ['rust.data']])
    print(opt)
